...
<Note>
  On the **desktop app**, programmatic rule scripts run in a secure sandbox by default (no
  filesystem or system access). To use Node.js APIs like `require`, `fs`, or `process`, turn on
  <a target="_blank" href="/interceptor/desktop-app/developer-script-mode">**Developer Script Mode**</a>.
</Note>
